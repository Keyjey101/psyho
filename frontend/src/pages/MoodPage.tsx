  ...
  return (
    <div className="min-h-screen bg-[#FAF6F1] p-6 lg:p-10">
      <div className="mx-auto max-w-3xl">
        ...
        {avg && (
          <div className="mb-8 rounded-2xl border border-[#E8DDD0] bg-white p-6 shadow-sm">
            <p className="text-sm text-[#8A7A6A]">Среднее настроение</p>
            <div className="mt-2 flex items-center gap-3">
              <span className="text-4xl">{MOOD_EMOJIS[Math.round(Number(avg))]}</span>
              <span className="text-3xl font-bold text-[#5A5048]">{avg}</span>
              <span className="text-[#B8A898]">из 5</span>
            </div>
          </div>
        )}
        ...
        {entries.length > 0 && (
          <div className="mb-8 rounded-2xl border border-[#E8DDD0] bg-white p-6 shadow-sm">
            <h2 className="mb-4 text-lg font-semibold text-[#5A5048]">История</h2>
            <div className="flex h-32 items-end gap-2">
              {entries.slice().reverse().slice(-20).map((entry) => (
                <div key={entry.id} className="flex flex-1 flex-col items-center gap-1">
                  ...
                  <div
                    className="w-full rounded-t bg-[#B8785A] transition-all"
        ...
        {entries.length === 0 && (
          <div className="rounded-2xl border border-[#E8DDD0] bg-white p-8 text-center shadow-sm">
            <p className="text-[#8A7A6A]">Пока нет записей. Оценивай настроение после каждой сессии!</p>
          </div>
        )}
      </div>
    </div>
